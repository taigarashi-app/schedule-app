...
function showPage(page) {
  ...
  // ========================================
  // ★ここから追加：下部メニューの文字色を連動して切り替える
  // ========================================
  document.querySelectorAll(".nav-item").forEach(item => {
    item.classList.remove("active"); // 一旦すべてのアクティブ状態を外す
  });

  // タッチされたページに対応するメニュー項目に active クラスを付与する
...
function renderCalendar() {
  ...
  for (let day = 1; day <= lastDay; day++) {
    ...
    // 予定の取得（cellKeyを使用）
    const dayEvents = schedules[cellKey]?.events || [];
...
showPage('calendarPage'); // 最初を開いたページに合わせる（または初期アクティブの設定）
